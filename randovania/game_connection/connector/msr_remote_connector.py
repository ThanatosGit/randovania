...
class MSRRemoteConnector(MercuryConnector):
    ...
    async def game_specific_execute(
        ...
        # same coop world => use the region name, otherwise use an empty string
        if provider_uuid == str(self._layout_uuid):
            region_name_send = f'"{region_name}"'
        else:
            region_name_send = '""'
        ...
        lua_code = get_lua_for_item(items_list, region_name_send)
